...
from filter_history import handle_history


class myWebChatModel(ChatModel):
    ...
    def predict(
        ...
        if(len(history)>0):
            self.consisthistory.append(history[-1])
        history = handle_history(self.consisthistory,query)
